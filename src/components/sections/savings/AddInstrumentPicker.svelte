    ...
    {#each templatesForLocale(app.ui.language) as t (t.id)}
      <li class="picker-row" class:selected={selected === t.id}>
        <button type="button" class="picker-btn" onclick={() => pick(t)}>
          ...
          <span class="picker-chev" aria-hidden="true">{expanded === t.id ? '▾' : '▸'}</span>
        </button>
        {#if expanded === t.id}
...
<style>
  ...
  .picker-btn { display: grid; grid-template-columns: 16px 1fr auto 16px; gap: var(--gap-3); width: 100%; padding: var(--gap-3); background: none; border: none; text-align: left; cursor: pointer; align-items: center; color: var(--fg); }
  ...
  .picker-chev { color: var(--fg-4); font-family: var(--mono); font-size: var(--t-sm); }
  ...
  @media (max-width: 480px) {
    .picker-btn { grid-template-columns: 16px 1fr 16px; }
